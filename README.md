...
│
├── notebooks/
│   └── C_fluminea_mass_balance_study.ipynb           # Full mass-balance notebook (5 cells)
│
├── scripts/                                           # WTP arsenic analysis (Fig. 2 + SI)
│   ├── 02_wtp_arsenic_analysis.py                    # Driver attribution -> intermediates
│   └── 03_figures.py                                 # Publication figures (PNG + PDF)
│
├── figures/                                           # Generated by 03_figures.py
│
└── data/                                             # Input data (see Data section below)
    ...
    ├── hcc_arsenic_tidy.csv                          # Cleaned WTP operational record
    ├── wtp_arsenic_data_dictionary.csv              # Column dictionary for the above
    └── wtp_arsenic_qaqc_log.txt                     # Cleaning / QA-QC provenance log
```
...
## WTP Arsenic Analysis (Figure 2 + Supporting Information)

The `scripts/` directory contains the driver-attribution analysis of the Hamilton (Waiora) WTP operational record that underpins **Figure 2** and the arsenic Supporting Information. It is independent of the mass-balance notebook and is run as a two-step pipeline from the repository root:

```bash
python scripts/02_wtp_arsenic_analysis.py   # 1. analysis -> data/wtp_*.{csv,json}
python scripts/03_figures.py                # 2. figures  -> figures/FigS1..S5.{png,pdf}
```
...
| Figure | Content |
|--------|---------|
| FigS1 | Treatment performance time series (in/out As, MAV, removal %) |
| FigS2 | Spearman correlation heatmap (drivers × As metrics) |
| FigS3 | PCA biplot (PC1 vs PC2), coloured by As removal % |
| FigS4 | Driver attribution: OLS coefficients + RF permutation importance (Fig. 2) |
| FigS5 | Mechanistic scatter: removal % vs Ca and vs Fe, by coagulant era |

The cleaning and QA/QC provenance for `hcc_arsenic_tidy.csv` (censoring, outlier flags, duplicate resolution) is recorded in `data/wtp_arsenic_qaqc_log.txt`; every column is defined in `data/wtp_arsenic_data_dictionary.csv`.
...
The WTP arsenic scripts additionally require:
```
scikit-learn
statsmodels
```
...
```bash
pip install pandas numpy matplotlib scipy openpyxl scikit-learn statsmodels
```
...
- **WTP operational record** (`hcc_arsenic_tidy.csv`) — Cleaned Hamilton (Waiora) WTP intake/outlet monitoring compilation, 2005–2025 (278 paired arsenic events plus raw-water covariates), used by the `scripts/` arsenic analysis. Provenance and QA/QC in `wtp_arsenic_qaqc_log.txt`; column definitions in `wtp_arsenic_data_dictionary.csv`.
