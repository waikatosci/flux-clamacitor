...
# Driver models for Figure 2 use a coverage-based covariate/row selection
# followed by median imputation and standardisation, rather than naive listwise
# deletion over all analytes (the 20-year record has uneven analyte coverage):
...
# With the default thresholds and the supplied dataset this yields a model
# sample of n = 107, the covariate set (Ca, Fe, Mn, Al, TOC, algae, pH,
# temperature, alkalinity), and the driver model reported in the manuscript
# (R2 ~ 0.33; Ca the sole significant positive
# predictor; alkalinity/pH/temperature significant negatives). The selection
# sample size (n) is computed at runtime and printed below; confirm the printed
# n and R2 against the manuscript. Both thresholds are exposed here for full
# transparency and are set to the values used in the paper.
...
# Coverage-based covariate/row selection (see header), then median imputation
# + standardisation.
